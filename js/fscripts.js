...
function goFlamingo(p0,p1) {
	...
	if (installedVersion['major'] >= requiredVersion) {
		var so = new SWFObject("flamingo/flamingo.swf", "flamingo", "100%", "100%", requiredVersion, "#ffffff");
...
function flamingo_onInit() {
	...
	var pc    = getURLParam("pc");     // e.g. pc=9801cs (a postcode)
	...
	var gdb = getURLParam("gdb");
	var gdb_legend = getURLParam("gdb_legend");
	...
	// load extra configuration file for a municipality or province
	if (gem.length > 0) {
	...
	if (gdb.length > 0) {
		...
		app.call("flamingo","setConfig", "<?xml version=\"1.0\" encoding=\"utf-8\"?><FLAMINGO xmlns:fmc=\"fmc\"><fmc:Legend id=\"legend\" symbolpath=\"../../../../output\"listento=\"geoportaal\" shadowsymbols=\"false\" right=\"right\" top=\"0\" bottom=\"320\" left=\"left +2\" backgroundcolor=\"#FFFFFF\" listento=\"map\"><group label=\"Kaartlaag\" open=\"true\"><item label=\"" + gdb + "\" canhide=\"false\" listento=\"geoportaal." + vis + "\"><symbol label=\"\" url=\"" + gdb_legend + "\"/></item></group></fmc:Legend></FLAMINGO>");
	}
	...
	if (ext.length > 0) {
		...
		app.call("flamingo" , "setArgument", "locationfinder" , "find" , "postcode,"+pc);
